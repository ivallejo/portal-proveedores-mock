...
export class DocumentoService {
  readonly documents = signal<Documento[]>([
    ...
    this.seed(
      1003,
      'E001-000091',
      'Servicios Marítimos del Pacífico',
      'P-2040',
      'Naviera Transoceánica S.A.',
      'Documento especial',
      'Aprobado',
  ...
  private seed(
    ...
    return {
      ...
      registeredAt: now,
      validation: {
        sunat: tipo === 'Con Orden de Compra' ? 'Aprobado' : 'Pendiente',
        sertica: tipo === 'Con Orden de Compra' ? 'Aprobado' : 'No aplica',
      },
      history: [
  ...
  registrarDocumento(dto: RegistroDocumentoDto): Observable<Documento> {
    const duplicate = this.documents().some(
      (item) =>
        ...
        item.sociedad === dto.sociedad,
    );
    if (duplicate)
      return throwError(
        () => new Error('Ya existe un documento con el mismo número, proveedor y sociedad.'),
      ).pipe(delay(700));
    const item: Documento = {
      ...
      validation: { sunat: 'Pendiente', sertica: 'Pendiente' },
      history: [{ status: 'Registrado', date: new Date().toISOString(), user: dto.proveedor }],
      details: dto.details,
    };
  ...
  private validateWithOc(item: Documento): Observable<Documento> {
    ...
    return forkJoin({
      sunat: of<'Observado' | 'Aprobado'>(
        item.numero.includes('FAIL') ? 'Observado' : 'Aprobado',
      ).pipe(delay(900)),
      sertica: of<'Observado' | 'Aprobado'>(
        item.oc?.includes('FAIL') ? 'Observado' : 'Aprobado',
      ).pipe(delay(1200)),
    }).pipe(
      map((result) => {
        const ok = result.sunat === 'Aprobado' && result.sertica === 'Aprobado';
        this.updateValidation(item.id, result.sunat, result.sertica);
        this.changeStatus(
          item.id,
          ok ? 'Pendiente de contabilización' : 'Devuelto al proveedor',
          'Sistema mock',
          ok ? undefined : 'Falló la validación SUNAT o Sertica.',
        );
  ...
  private validateWithoutOc(item: Documento, validateSunat: boolean): Observable<Documento> {
    this.changeStatus(item.id, 'En validación', 'Sistema mock');
    return of<'Observado' | 'Aprobado' | 'No aplica'>(
      validateSunat ? (item.numero.includes('FAIL') ? 'Observado' : 'Aprobado') : 'No aplica',
    ).pipe(
      delay(1000),
      map((sunat) => {
        this.updateValidation(item.id, sunat, 'No aplica');
        this.changeStatus(item.id, 'Pendiente de aprobación', 'Sistema mock');
        return this.get(item.id)!;
      }),
    );
  }
  changeStatus(id: number, status: DocumentStatus, user: string, comment?: string): void {
      ...
      items.map((item) =>
        item.id === id
          ? {
              ...
              status,
              comment,
              history: [...item.history, { status, comment, user, date: new Date().toISOString() }],
            }
          : item,
      ),
    );
  ...
  updateValidation(
    id: number,
    sunat: Documento['validation']['sunat'],
    sertica: Documento['validation']['sertica'],
  ): void {
    this.documents.update((items) =>
      items.map((item) =>
        item.id === id ? { ...item, validation: { ...item.validation, sunat, sertica } } : item,
      ),
    );
  }
  get(id: number): Documento | undefined {
    return this.documents().find((item) => item.id === id);
  }
  mutateForAccounting(id: number, ruta: 'SAP' | 'Sertica', numero: string): void {
    this.mutate(id, (item) => ({ ...item, contabilizacion: { ruta, numero } }));
  }
  ...
  markEscalated(): void {
    this.documents.update((items) =>
      items.map((item) =>
        ...
        !item.escalated &&
        Date.now() - item.registeredAt > 30000
          ? {
              ...item,
              escalated: true,
              aprobador: 'Jefatura de Área',
              history: [
                ...item.history,
                {
                  status: item.status,
                  ...
                  user: 'Sistema mock',
                  comment: 'Escalamiento automático por tiempo excedido.',
                },
              ],
            }
          : item,
