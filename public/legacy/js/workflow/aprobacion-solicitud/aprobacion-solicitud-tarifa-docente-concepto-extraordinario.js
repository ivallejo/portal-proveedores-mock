...
class AprobacionSolicitudTarifaDocenteConceptoExtraordinario {
    constructor() {
        ...
        this.dataTable = $("#frmTarifaDocente #tblTarifas").DataTable({
            ...
            order: [],
            columns: [
                { data: "idTarifaDocenteConceptoExtraordinarioSolicitud", title: "ID Tarifa", className: "no-mobile" },
                ...
                { data: "fechaTermino", title: "Fecha Término", render: (data, type, row) => Util.formatearFechaAsString(row.fechaTermino) },
    ...
    cargarData(solicitud, esEvaluador, esInicio = false) {
        let divAcciones = $("#frmTarifaDocente .row.mt-auto");
        if (esEvaluador) {
